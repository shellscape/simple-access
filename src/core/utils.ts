...
export class Utils {
   ...
   * @returns {any}
   */
  private static resolveConditionParam(param: any, subject: Tuple, resource: Tuple) {
    let path;
    let data;
   ...
   * @returns object
   */
  private static parseCondition(condition: Tuple, subject: Tuple, resource: Tuple) {
      ...
      for (let i = 0; i < conditions.length; i += 1) {
        ...
        try {
          if (!_.isObject(condition)) {
            throw new ErrorEx(ErrorEx.VALIDATION_ERROR, 'Condition must be an object');
          }

          const [[key, query]] = Object.entries(condition);
          const pKey = this.resolveConditionParam(key, subject, resource);
          const pQuery = this.parseCondition(query, subject, resource);
